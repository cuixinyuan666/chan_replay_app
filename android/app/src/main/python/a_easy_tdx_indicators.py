...
def _rolling_mean(values: list[float | None], n: int) -> list[float | None]:
    return [None if (rows := _window(values, i, n)) is None else sum(rows) / n for i in range(len(values))]


def _rolling_sum(values: list[float | None], n: int) -> list[float | None]:
    return [None if (rows := _window(values, i, n)) is None else sum(rows) for i in range(len(values))]
...
def _bias(close: list[float | None], l1: int = 6, l2: int = 12, l3: int = 24) -> list[dict[str, float | None]]:
    ...
    return [{f'BIAS{j + 1}': _rd(None if c is None or mas[j][i] is None or abs(mas[j][i] or 0) < 1e-12 else (c - (mas[j][i] or 0)) / (mas[j][i] or 1) * 100.0) for j in range(3)} for i, c in enumerate(close)]
...
def _obv(close: list[float | None], vol: list[float | None]) -> list[dict[str, float | None]]:
    ...
    for i, c in enumerate(close):
        if i > 0 and c is not None and close[i - 1] is not None and vol[i] is not None:
            if c > close[i - 1]: total += vol[i] or 0.0
            elif c < close[i - 1]: total -= vol[i] or 0.0

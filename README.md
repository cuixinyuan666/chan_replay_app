...
4. chanpy_runtime.py 会调用 CChan，导出 FX / BI / SEG / ZS。
...
10. Android Chaquopy 新增 python_chan MethodChannel。
11. Android chanpy_runtime.py 已接入 CChan，尝试导出 FX / BI / SEG / ZS。

待完成：
1. Flutter 直接消费 step frames。
2. 验证 Android APK 内 python/chan.py 打包路径和 CSV 写入权限。
3. BSP 买卖点导出与绘制。
4. Python 返回真实 merged_bars。
5. Flutter 本地 CSV 上传到 /api/chan/analyze_bars。
6. 删除或隔离旧 Dart 算法层。
7. 跑 flutter analyze / flutter run，修编译和运行期问题。
```

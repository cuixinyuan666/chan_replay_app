#!/usr/bin/env python3
"""Refine toolbar and TradingView toolbox interactions.

This patch addresses the follow-up interaction corrections:
- Left toolbar becomes an overlay edge drawer instead of consuming chart width.
- Collapsed toolbar leaves a small tail for mouse/touch reveal.
- TV toolbox is opened from the left toolbar; the chart-side floating TV button is hidden.
- TV toolbox adds a left quick tool rail that accepts dragged tools.
- TV toolbox uses an interactive RawScrollbar with a ScrollController.
- Tool groups and tools can be reordered by drag.
- The top operation guide is moved behind a ! dialog button.
- Tooltips on groups/tools wait 3 seconds before showing.
- Chan overlay group is placed at the top by default.

Run:
  python tools/apply_toolbar_toolbox_drag_refinements.py --check
  python tools/apply_toolbar_toolbox_drag_refinements.py --apply
  flutter analyze
"""
...
def patch_page(src: str):
    changed = False
    notes = []
    if '_tvToolboxOpenSignal' not in src:
        src, did, note = replace_once(
            src,
            '  bool _toolbarHovering = false;\n',
            '  bool _toolbarHovering = false;\n  final ValueNotifier<int> _tvToolboxOpenSignal = ValueNotifier<int>(0);\n',
            'page tv toolbox signal field',
        )
        changed |= did
        notes.append(note)
    else:
        notes.append('OK page tv toolbox signal field')
    if '_tvToolboxOpenSignal.dispose();' not in src:
        src, did, note = replace_once(
            src,
            '    _loadVisualTimer?.cancel();\n',
            '    _loadVisualTimer?.cancel();\n    _tvToolboxOpenSignal.dispose();\n',
            'page dispose tv toolbox signal',
        )
        changed |= did
        notes.append(note)
    else:
        notes.append('OK page dispose tv toolbox signal')
...
'''
    src, did, note = replace_method(src, '  @override\n  Widget build(BuildContext context) {', build_method, 'page overlay build layout')
    changed |= did
    notes.append(note)
...
'''
    src, did, note = replace_method(src, '  Widget _buildLeftToolbar() {', left_toolbar, 'page edge drawer left toolbar')
    changed |= did
    notes.append(note)
...
def patch_chart(src: str):
    changed = False
    notes = []
    if 'final ValueListenable<int>? toolboxOpenSignal;' not in src:
        src, did, note = replace_once(
            src,
            '  final ValueChanged<TradingViewDrawingTool>? onChanOverlayToggled;\n',
            '  final ValueChanged<TradingViewDrawingTool>? onChanOverlayToggled;\n  final ValueListenable<int>? toolboxOpenSignal;\n',
            'chart toolbox signal field',
        )
        changed |= did
        notes.append(note)
    else:
        notes.append('OK chart toolbox signal field')
    if 'this.toolboxOpenSignal,' not in src:
        src, did, note = replace_once(
            src,
            '    this.onChanOverlayToggled,\n',
            '    this.onChanOverlayToggled,\n    this.toolboxOpenSignal,\n',
            'chart toolbox signal constructor',
        )
        changed |= did
        notes.append(note)
    else:
        notes.append('OK chart toolbox signal constructor')
    if 'openSignal: widget.toolboxOpenSignal,' not in src:
        src, did, note = replace_once(
            src,
            '      selectedTool: _selectedDrawingTool,\n',
            '      selectedTool: _selectedDrawingTool,\n      openSignal: widget.toolboxOpenSignal,\n',
            'chart pass toolbox signal to host',
        )
        changed |= did
        notes.append(note)
    else:
        notes.append('OK chart pass toolbox signal to host')
    return src, changed, notes
...
def main() -> int:
    ...
    any_changed = False
    all_notes = []
    for path, patcher in ((PAGE, patch_page), (CHART, patch_chart)):
    ...
    all_notes.append(f'{TOOLBOX}: {"APPLY" if toolbox_changed else "OK"} replace drag-capable toolbox')

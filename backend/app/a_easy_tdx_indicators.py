...
def _indicator_point(row: dict[str, Any], fallback: int, value: Any) -> dict[str, Any]:
    return {
        'time': _time(row),
        'raw_index': _raw_index(row, fallback),
...
def _boll(close_values: list[float | None], window: int) -> list[tuple[float | None, float | None, float | None]]:
...
def build_easy_tdx_indicators(
    ...
    *,
    ...
    """Build display-only indicators from easy-tdx bars.

    The result is for Flutter sub-chart rendering and tooltip display only. It
    must not feed chan.py FX / BI / SEG / ZS / BSP calculation.
    """
    close_values = [_to_float(row.get('close') or row.get('c')) for row in bars]
    result: dict[str, Any] = {
        'vol': [
            _indicator_point(row, i, row.get('volume', row.get('vol', row.get('v'))))
            for i, row in enumerate(bars)
        ],
        'amount': [
            _indicator_point(row, i, row.get('amount', row.get('money')))
            for i, row in enumerate(bars)
        ],
        'turnover': [
            _indicator_point(row, i, row.get('turnover', row.get('turnover_rate')))
            for i, row in enumerate(bars)
        ],
        'ma': {},
    }
    ...
    result['ma'] = {
        str(window): [
            {'time': _time(row), 'raw_index': _raw_index(row, i), 'value': value}
            for i, (row, value) in enumerate(zip(bars, _moving_average(close_values, window)))
        ]
    ...
    result['boll'] = [
        {
            'time': _time(row),
            'raw_index': _raw_index(row, i),
            'upper': upper,
            'mid': mid,
            'lower': lower,
        }
        for i, (row, (upper, mid, lower)) in enumerate(zip(bars, _boll(close_values, boll_window)))
    ]
    ...
    result['macd'] = [
        ...
        for i, (row, (dif, dea, hist)) in enumerate(
            zip(bars, _macd(close_values, fast=macd_fast, slow=macd_slow, signal=macd_signal))
        )
    ]
    return result

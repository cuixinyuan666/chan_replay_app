...
class ChipDistributionBar {
  ...
  bool get hasExactChipBins =>
      priceSellVolume.isNotEmpty || priceBuyVolume.isNotEmpty || priceVolume.isNotEmpty;
  ...
  factory ChipDistributionBar.fromJson(Map<String, dynamic> row, int fallbackIndex) {
    final tickBins = ChipTickBins.fromJson(row['chip_tick_bins'] ?? row['chipTickBins']);
    return ChipDistributionBar(
      index: _int(row['x'] ?? row['index'] ?? row['idx']) ?? fallbackIndex,
      time: _parseTime(row['dt'] ?? row['datetime'] ?? row['date'] ?? row['time'] ?? row['t']),
...
class ChipTickBins {
  ...
  bool get isEmpty => sellByPrice.isEmpty && buyByPrice.isEmpty && totalByPrice.isEmpty;
...
class ChipDistributionEngine {
  ...
  void _foldExactPriceVolume({
    ...
    for (final entry in bar.priceSellVolume.entries) {
      ...
      final idx = ((price - minPrice) / step).floor().clamp(0, sellWeights.length - 1).toInt();
    ...
    for (final entry in bar.priceBuyVolume.entries) {
      ...
      final volume = _safeWeight(entry.value) * ageWeight;
      if (price <= 0 || volume <= 0) continue;
      final idx = ((price - minPrice) / step).floor().clamp(0, buyWeights.length - 1).toInt();
  ...
  void _foldOhlcvTriangular({
    ...
    final from = ((low - minPrice) / step).floor().clamp(0, buyWeights.length - 1).toInt();
    final to = ((high - minPrice) / step).floor().clamp(0, buyWeights.length - 1).toInt();
    ...
    if (kernelSum <= 0) {
      final idx = ((center - minPrice) / step).floor().clamp(0, buyWeights.length - 1).toInt();
      buyWeights[idx] += volume;
      return;
    }

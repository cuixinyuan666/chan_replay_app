                  ...
                  children: [
                    for (final group in groups)
                      _ToolGroupTile(
                        ...
                        group: group,
                        tools: grouped[group] ?? const <TradingViewDrawingToolMeta>[],
...
class _ToolGroupTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return ExpansionTile(
      maintainState: true,
      initiallyExpanded:
          group == TradingViewDrawingGroup.chanOverlay || group == TradingViewDrawingGroup.lines,
          ...
          children: [
            ListTile(
              ...
              title: Text(meta.label,
                  style: TextStyle(
                      color: enabled ? Colors.white : Colors.white60, fontSize: 13)),
            ...
            if (isEasyIndicator) _EasyIndicatorControls(
              enabled: enabled,
              keys: indicatorKeys,
              enabledIndicators: enabledIndicators,
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              const Text('副图数量', style: TextStyle(color: Colors.white54, fontSize: 11)),
              IconButton(
                tooltip: '减少副图',
                onPressed: enabled && onPanelCountChanged != null
                    ? () => onPanelCountChanged!((panelCount - 1).clamp(0, 4).toInt())
                    : null,

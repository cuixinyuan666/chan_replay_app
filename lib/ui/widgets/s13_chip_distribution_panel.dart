          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: <Widget>[
                Row(
                  children: <Widget>[
                    const Icon(Icons.stacked_bar_chart, color: Color(0xFF8AB4FF), size: 18),
                ...
                Wrap(
                  ...
                  children: <Widget>[
                    ...
                    _miniMetric('目标', bars.isEmpty ? '-' : '${result.targetIndex + 1}/${bars.length}'),
                    ...
                    _miniMetric('获利', '${(result.profitRatio * 100).toStringAsFixed(1)}%'),
                  ],
                ),
                ...
                Text(
                  ...
                  style: TextStyle(color: Colors.white.withValues(alpha: 0.48), fontSize: 10.5),
                ),
              ],
            ),
          ),
...
class _CompactChipDistributionPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final nonZero = result.bins.where((b) => b.weight > 0).toList(growable: false);
    ...
    final sellPaint = Paint()..color = const Color(0xFF26A69A).withValues(alpha: 0.68);
    final buyPaint = Paint()..color = const Color(0xFFEF5350).withValues(alpha: 0.68);
    final pocPaint = Paint()..color = const Color(0xFFFFB300).withValues(alpha: 0.88);
    ...
    for (var i = 0; i < result.bins.length; i++) {
      ...
      canvas.drawRect(Rect.fromLTWH(left, y0, sellW, h), isPoc ? pocPaint : sellPaint);
      canvas.drawRect(Rect.fromLTWH(left + sellW, y0, buyW, h), isPoc ? pocPaint : buyPaint);
    }

    final currentY = _priceToY(result.currentPrice, minPrice, maxPrice, top, bottom);
    canvas.drawLine(Offset(left, currentY), Offset(right, currentY), currentPaint);
    _drawText(canvas, maxPrice.toStringAsFixed(2), Offset(4, top), Colors.white54);
    _drawText(canvas, minPrice.toStringAsFixed(2), Offset(4, bottom - 12), Colors.white54);
    _drawText(canvas, '现价', Offset(right - 28, currentY - 14), const Color(0xFF66BB6A));
  }
  ...
  @override
  bool shouldRepaint(covariant _CompactChipDistributionPainter oldDelegate) => oldDelegate.result != result;
}

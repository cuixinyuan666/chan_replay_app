...
class MultiLevelStrategySignalSelection {
  ...
  String get markerId =>
      's7_strategy_signal_marker_${ruleModeName}_${highLevel}_${highRawIndex}_${lowLevel}_$lowRawIndex';
...
class MultiLevelIntervalSignalPanel extends StatefulWidget {
  ...
  @override
  State<MultiLevelIntervalSignalPanel> createState() =>
      _MultiLevelIntervalSignalPanelState();
}

class _MultiLevelIntervalSignalPanelState
    extends State<MultiLevelIntervalSignalPanel> {
  int _selectedIndex = 0;
  int _pairIndex = 0;
  String _ruleMode = 'strategy';
  String _strategyRuleName = 'DAILY_2B_MIN30_1B';
  String _directionFilter = 'same';
  final String _highTypeFilter = 'ANY';
  final String _lowTypeFilter = 'ANY';
  String _lastEmittedKey = '';

  bool get _isScanMode => widget.mode == 'signal_scan_once';
  bool get _isStrategyMode => _ruleMode == 'strategy';
  String get _signalRuleMode =>
      _isStrategyMode ? 'strategy_interval_nest_buy' : 'validation_any_bsp_pair';
  String get _ruleChipLabel =>
      _isStrategyMode ? 'strategy interval buy' : 'any BSP pair';
  ...
  String _runtimePath(Map<String, dynamic>? log) {
    final value =
        '${log?['runtime_path'] ?? widget.snapshot.meta['runtime_path'] ?? 'high_speed'}'
            .trim();
  ...
  List<_LevelPair> get _pairs {
    ...
    for (final r in widget.snapshot.relations) {
      relationPairs['${r.parentLevel}->${r.childLevel}'] =
          _LevelPair(r.parentLevel, r.childLevel);
    }
    if (relationPairs.isNotEmpty) return relationPairs.values.toList();
    final levels = widget.snapshot.levels;
    return [
      for (var i = 0; i < levels.length - 1; i++)
        _LevelPair(levels[i], levels[i + 1])
    ];
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (_selectedIndex >= signals.length) {
      _selectedIndex = signals.isEmpty ? 0 : signals.length - 1;
    }
    final selected = signals.isEmpty ? null : signals[_selectedIndex];
    final selection = selected == null || pair == null
        ? null
        : _toSelection(pair, selected);
    ...
    return Container(
      ...
      child: Wrap(
        ...
        children: [
          ...
          _chip('runtime_path', _runtimePath(_timeLog),
              _runtimePath(_timeLog) != 'slow_path'),
          ...
          if (selected != null)
            _chip('selected', '${_selectedIndex + 1}/${signals.length}', true),
          if (selected != null) _chip('state', selected.state, selected.confirmed),
          if (selected != null)
            _chip('pattern', '${selected.highBsp.type}->${selected.lowBsp.type}', true),
          _smallButton('Prev',
              signals.isEmpty ? null : () => _setSelected(_selectedIndex - 1, signals.length)),
          _smallButton('Next',
              signals.isEmpty ? null : () => _setSelected(_selectedIndex + 1, signals.length)),
          OutlinedButton.icon(
            onPressed: selection == null
                ? null
                : () => widget.onJumpToSignal?.call(selection),
          ...
          OutlinedButton.icon(
            onPressed: () => _copy(context,
                _copyS1EvidenceText(pair, signals, selected, selection), 'S1/S7 evidence copied'),
          ...
          OutlinedButton.icon(
            onPressed: () => _copy(context,
                _copySignalText(pair, signals, selected, selection), 'Debug signal diagnostics copied'),
  ...
  List<_SignalMatch> _buildSignals(_LevelPair? pair) {
    ...
    if (high == null || low == null || high.bsps.isEmpty || low.bsps.isEmpty) {
      return const [];
    }
    ...
    for (final highBsp in high.bsps.where(_matchesHighFilter)) {
      ...
      final childStart =
          relations.map((r) => r.childStartRawIndex).reduce((a, b) => a < b ? a : b);
      final childEnd =
          relations.map((r) => r.childEndRawIndex).reduce((a, b) => a > b ? a : b);
      for (final lowBsp in low.bsps.where(
          (b) => _matchesLowFilter(b, highBsp, childStart, childEnd))) {
  ...
  bool _matchesLowFilter(
      BspPoint lowPoint, BspPoint highPoint, int childStart, int childEnd) {
    if (lowPoint.rawIndex < childStart || lowPoint.rawIndex > childEnd) return false;
    if (_isStrategyMode) {
      return _strategyHighTypes.contains(highPoint.type) &&
          _strategyLowTypes.contains(lowPoint.type);
    }
    ...
    if (_directionFilter == 'mixed') {
      return (highPoint.isBuy && lowPoint.isSell) ||
          (highPoint.isSell && lowPoint.isBuy);
    }
    return (highPoint.isBuy && lowPoint.isBuy) ||
        (highPoint.isSell && lowPoint.isSell);
  }

  LevelRelation? _relationContaining(List<LevelRelation> relations, int childRawIndex) {
    for (final relation in relations) {
      if (childRawIndex >= relation.childStartRawIndex &&
          childRawIndex <= relation.childEndRawIndex) {
        return relation;
      }
    }
    return null;
  ...
  MultiLevelStrategySignalSelection _toSelection(_LevelPair pair, _SignalMatch selected) {
    return MultiLevelStrategySignalSelection(
      ...
      sourceBspIdentifiers:
          '${pair.parentLevel}#${selected.highBsp.index}:raw=${selected.highBsp.rawIndex}:type=${selected.highBsp.type};${pair.childLevel}#${selected.lowBsp.index}:raw=${selected.lowBsp.rawIndex}:type=${selected.lowBsp.type}',
      sourceTargetLevels: '${pair.parentLevel}->${pair.childLevel}',
      nativeRelationRange:
          'parent=${selected.relation.parentRawIndex}:child=${selected.relation.childStartRawIndex}-${selected.relation.childEndRawIndex}',
      strictStepVisibility: _isScanMode
          ? 'scan once snapshot; not strict step confirmation'
  ...
  String _copyS1EvidenceText(
    _LevelPair? pair,
    List<_SignalMatch> signals,
    _SignalMatch? selected,
    MultiLevelStrategySignalSelection? selection,
  ) {
    return [
  ...
  Set<String> get _strategyHighTypes =>
      _strategyRuleName == 'DAILY_2B_MIN30_1B' ? const {'B2', 'B2s'} : const {'B3', 'B3s'};
  Set<String> get _strategyLowTypes =>
      _strategyRuleName == 'DAILY_3B_MIN30_2B' ? const {'B2', 'B2s'} : const {'B1'};
  ...
  Widget _ruleModeDropdown() => _dropdown<String>(
        'rule mode',
        _ruleMode,
  ...
  Widget _strategyRuleDropdown() => _dropdown<String>(
        'strategy rule',
        _strategyRuleName,
        _strategyRules,
        (v) {
  ...
  Widget _pairDropdown() {
    ...
    return _dropdown<int>(
      'level pair',
      _pairIndex.clamp(0, pairs.length - 1).toInt(),
      ...
      width: 150,
      labelFor: (v) => pairs[v].label,
    );
  }

  Widget _directionDropdown() => _dropdown<String>(
        'direction',
        _directionFilter,
        const ['all', 'same', 'buy', 'sell', 'mixed'],
        (v) {
          setState(() {
      ...
      child: DropdownButtonFormField<T>(
        ...
        items: [
          for (final item in values)
            DropdownMenuItem<T>(
                value: item, child: Text(labelFor == null ? '$item' : labelFor(item)))
        ],
        onChanged: onChanged,
      ),
    );
  ...
  Widget _smallButton(String text, VoidCallback? onPressed) =>
      OutlinedButton(onPressed: onPressed, style: _debugButtonStyle(), child: Text(text));

  ButtonStyle _primaryButtonStyle() => OutlinedButton.styleFrom(
  ...
  ButtonStyle _debugButtonStyle() => OutlinedButton.styleFrom(
        foregroundColor: const Color(0xFF9AA0A6),
        side: const BorderSide(color: Color(0x449AA0A6)),
        padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),

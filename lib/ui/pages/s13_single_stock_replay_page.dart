...
class _S13SingleStockReplayPageState extends State<S13SingleStockReplayPage> {
  ...
  List<BspPoint> _bspCandidateTrail(ChanSnapshot current) {
    return _bspCandidateTrailForLevel(_activeLevel, current);
  }

  List<BspPoint> _bspCandidateTrailForLevel(
      String rawLevel, ChanSnapshot current) {
    final a = _analysis;
    if (!_showBspCandidateTrail ||
        !_isStepMode ||
        a == null ||
        a.frames.isEmpty) return const <BspPoint>[];
    final level = rawLevel.trim().toUpperCase();
    final live = current.bsps.map(_bspKey).toSet();
    final trail = <String, BspPoint>{};
    for (var i = 0; i <= _safeFrameIndex && i < a.frames.length; i++) {
      final s = a.frames[i].of(level);
      if (s == null) continue;
      for (final p in s.bsps) {
        final k = _bspKey(p);
        if (!trail.containsKey(k)) trail[k] = _trailPoint(p);
      }
    }
    for (final k in live) {
      trail.remove(k);
  ...
  String _bspKey(BspPoint p) {
    final side = p.isSell ? 'sell' : (p.isBuy ? 'buy' : 'bsp');
    return '${p.level.trim().toLowerCase()}|$side|${p.type.trim().toLowerCase()}|${p.rawIndex}|${p.biIndex ?? -1}|${p.segIndex ?? -1}|${p.zsIndex ?? -1}';
  }

  BspPoint _trailPoint(BspPoint p) {
    final t = p.type.trim().isEmpty ? 'BSP' : p.type.trim();
    return BspPoint(
        index: p.index,
        rawIndex: p.rawIndex,
        time: p.time,
  ...
  List<BspPoint> _snapshotBspsWithTrail(String level) {
    ...
    return <BspPoint>[..._bspCandidateTrailForLevel(level, snap), ...snap.bsps];
  }

  BspPoint? _bspAt(String level, int rawIndex) {
    final rows = _snapshotBspsWithTrail(level);
    for (final bsp in rows) {
      if (bsp.rawIndex == rawIndex) return bsp;
    }
    return null;
  }

  BspPoint? _firstBspInRange(String level, int start, int end) {
    final rows = _snapshotBspsWithTrail(level)
        .where((bsp) => bsp.rawIndex >= start && bsp.rawIndex <= end)
        .toList(growable: false)
      ..sort((a, b) => a.rawIndex.compareTo(b.rawIndex));
  ...
  LevelRelation? _relationDown(String parentLevel, int parentRawIndex) {
    ...
    final parentIndex = levels.indexOf(parentLevel);
    if (parentIndex < 0 || parentIndex >= levels.length - 1) {
      return null;
    }
  ...
  LevelRelation? _relationUp(String childLevel, int childRawIndex) {
    ...
    if (c == null) return null;
    final levels = _loadedLevels;
    final childIndex = levels.indexOf(childLevel);
    ...
    final matches = c.relations
        .where((r) =>
            r.parentLevel.trim().toUpperCase() == parentLevel &&
            r.childLevel.trim().toUpperCase() == childLevel &&
            r.childStartRawIndex <= childRawIndex &&
            r.childEndRawIndex >= childRawIndex)
        .toList(growable: false)
  ...
  int? _mapRawIndexToLevel(String fromLevel, int rawIndex, String toLevel) {
    final levels = _loadedLevels;
    final from = levels.indexOf(fromLevel);
    final to = levels.indexOf(toLevel);
  ...
  void _setFrameIndex(int index) {
    ...
    setState(() {
      ...
      _priceScale = 1.0;
      _status =
          'S13 step frame ${next + 1}/${a.frames.length} active:$level relations:${f.relations.length} nested_markers:${_nestedBspMarkers.length} candidate_trail:$_bspCandidateTrailCount';
    });
  }

  void _stepFrameBy(int d) => _setFrameIndex(_safeFrameIndex + d);
                    ...
                    if (_panelOpen)
                      Padding(
                          ...
                          child: SizedBox(
                              width: 370,
                              height: math.min(
                                  560, MediaQuery.sizeOf(context).height - 120),
  ...
  Widget _unifiedToolPanel() => ListView(
          ...
          children: <Widget>[
            ...
            Wrap(
                spacing: 6,
                runSpacing: 6,
                children: [for (final l in _levelOptions) _levelChip(l)]),
            const SizedBox(height: 8),
            Wrap(
                spacing: 6,
                runSpacing: 6,
                children: [for (final l in _loadedLevels) _activeLevelChip(l)]),
            const SizedBox(height: 8),
            Wrap(spacing: 8, runSpacing: 8, children: <Widget>[
  ...
  Widget _nestedBspMarkerButton(_NestedBspMarker marker) {
    return InkWell(
    ...
    return LayoutBuilder(builder: (context, constraints) {
      ...
      return Stack(children: <Widget>[
        _draggableReplayControl(
            ...
            child: _replayIcon(Icons.first_page, '第一帧',
        ...
        _draggableReplayControl(
            ...
            constraints: constraints,
            child: _replayIcon(_playing ? Icons.pause : Icons.play_arrow, '播放',
  ...
  Widget _speedButton() => PopupMenuButton<double>(
      ...
      child: Container(
          ...
          child: Text('${_playSpeed.toStringAsFixed(2)}x',
              style: const TextStyle(
                  color: Colors.white70,
                  fontSize: 11,
                  fontWeight: FontWeight.w700))));

  Widget _replayIcon(IconData icon, String tip, VoidCallback? onPressed,
          {double alpha = 0.82}) =>
      Tooltip(
          message: tip,
          child: IconButton(
              ...
              color: Colors.white
                  .withValues(alpha: onPressed == null ? 0.22 : alpha),
              style: IconButton.styleFrom(
  ...
  Widget _activeLevelChip(String l) {
    final selected = _activeLevel == l,
        canSelect = _currentSnapshot?.snapshots.containsKey(l) == true;
    return ChoiceChip(
        label: Text(l),
        selected: selected,
  ...
  Widget _modeChip(String v) {
    final selected = _mode == v;
    return ChoiceChip(
        label: Text(v),
        selected: selected,
        onSelected: _loading ? null : (_) => _setReplayMode(v),
        selectedColor: const Color(0xFFFFD54F),
        backgroundColor: const Color(0xFF20242E),
        labelStyle: TextStyle(
            color: selected ? Colors.black : Colors.white70, fontSize: 12));
  ...
  Widget _indicatorChip(String n) {
    final selected = _enabledEasyTdxIndicators.contains(n);
    return FilterChip(
        label: Text(n),
        selected: selected,
        onSelected: (_) => _toggleEasyTdxIndicator(n),
        selectedColor: const Color(0xFFFFD54F),
        backgroundColor: const Color(0xFF20242E),
        labelStyle: TextStyle(
            color: selected ? Colors.black : Colors.white70, fontSize: 12));
  ...
  void _panChartByBars(int bars) {
    ...
    final max = s.rawBars.length - 1,
        current = _viewEndIndex ?? max,
        next = (current + bars).clamp(0, max).toInt();
  ...
  Widget _dateButton(
          String label, DateTime? value, VoidCallback onPressed) =>
      SizedBox(
          width: 132,
          child: OutlinedButton.icon(
              onPressed: _loading ? null : onPressed,
              icon: const Icon(Icons.calendar_month, size: 16),
              label: Text(value == null ? label : _fmtDate(value),
  ...
  Widget _runtimePathButton() => ValueListenableBuilder<RuntimePath>(
      ...
      builder: (context, path, _) => PopupMenuButton<RuntimePath>(
          ...
          itemBuilder: (context) => const <PopupMenuEntry<RuntimePath>>[
                PopupMenuItem(
                    value: RuntimePath.highSpeed, child: Text('高速路（默认）')),
                PopupMenuItem(
                    value: RuntimePath.slowPath, child: Text('慢速路（调试）')),
              ],
          child: Container(
              width: 178,
  ...
  String _runtimePathText(PythonMultiLevelChanAnalysis a) {
    final raw =
        '${a.meta['runtime_path'] ?? a.snapshot.meta['runtime_path'] ?? RuntimePathController.current.wireName}'
            .trim();

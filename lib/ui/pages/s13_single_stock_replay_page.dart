...
class _S13SingleStockReplayPageState extends State<S13SingleStockReplayPage> {
  ...
  List<_NestedBspMarker> get _nestedBspMarkers {
    ...
    triggers.sort((a, b) {
      ...
      final bi = levels.indexOf(b.sourceLevel);
  ...
  Widget _unifiedToolPanel() => ListView(
        ...
        children: <Widget>[
          ...
          _sectionTitle('图层'),
          Wrap(spacing: 6, runSpacing: 6, children: <Widget>[
            for (final name in const <String>['MA', 'BOLL', 'VOL', 'MACD'])
              _indicatorChip(name),
          ]),
          SwitchListTile(
            value: _showBspCandidateTrail,
            onChanged:
                _loading ? null : (v) => setState(() => _showBspCandidateTrail = v),
            dense: true,
  ...
  String _buildStatus(PythonMultiLevelChanAnalysis analysis, DateTime start, DateTime end) {
    final m = analysis.meta;
    return 'S13 analyze_multi ${_mode.toUpperCase()} runtime_path:${_runtimePathText(analysis)} native:${m['native_cchan_lv_list']} fallback:${m['fallback_to_bridge'] ?? false} frames:${analysis.frames.length} active_frame:$_stepFrameLabel levels:${analysis.snapshot.levels.join(',')} window:${_fmtDate(start)}~${_fmtDate(end)} nested_markers:${_nestedBspMarkers.length} candidate_trail:$_bspCandidateTrailCount missing_edges:${_missingAdjacentRelationEdges().join(',')}';
  }

...
class _OriginReplayPageState extends State<OriginReplayPage> {
  ...
  Future<void> _load() async {
    ...
    final source = PythonChanEngineSource(baseUrl: _backendUrlController.text.trim());
    try {
      ...
      setState(() {
        ...
        _cursor = _isStepMode ? math.min(120, snapshot.rawBars.length).toInt() : snapshot.rawBars.length;
        _snapshot = _isStepMode ? _sliceSnapshot(snapshot, _cursor) : snapshot;
        _viewEndIndex = null;
        _crosshairIndex = null;
        _priceScale = 1.0;
        _status = 'chan.py 获取${symbol.market}${symbol.code} ${_periodAdjustLabel} ${snapshot.rawBars.length}根 K线成功';
      });
            ...
            child: Padding(
              ...
              child: SingleChildScrollView(
                child: Column(
                  ...
                  children: [
                    const Text('Python chan.py 数据与计算', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
                    const SizedBox(height: 8),
                    const Text('Flutter 只负责显示；FX / BI / SEG / ZS 均来自 Python chan.py JSON。', style: TextStyle(color: Colors.white70)),
                    const SizedBox(height: 12),
                    TextField(
                      controller: _backendUrlController,
                      enabled: !_loading,
                      decoration: const InputDecoration(labelText: 'Python chan.py 本地服务地址', hintText: 'http://127.0.0.1:8000', border: OutlineInputBorder()),
                    ),
                    const SizedBox(height: 10),
                    TextField(
                      controller: _stockCodeController,
                      enabled: !_loading,
                      decoration: const InputDecoration(labelText: '代码（自动识别市场）', hintText: '000001 / 600000 / SZ000001 / 600000.SH', border: OutlineInputBorder()),
                    ),
                    const SizedBox(height: 10),
                    DropdownButtonFormField<String>(
                      ...
                      onChanged: _loading ? null : (v) => setSheetState(() => mode = v ?? mode),
                    ),
                    const SizedBox(height: 10),
                    Row(children: [
                      Expanded(child: DropdownButtonFormField<String>(
                        ...
                        onChanged: _loading ? null : (v) => setSheetState(() => period = v ?? period),
                      )),
                      ...
                      Expanded(child: DropdownButtonFormField<String>(
                        ...
                        onChanged: _loading ? null : (v) => setSheetState(() => adjust = v ?? adjust),
                      )),
                    ]),
                    const SizedBox(height: 10),
                    Row(children: [
                      Expanded(child: _dateTile('开始日期', start, !_loading, () async {
                        final picked = await _pickDate(start);
                        if (picked != null) setSheetState(() => start = picked);
                      })),
                      const SizedBox(width: 10),
                      Expanded(child: _dateTile('结束日期', end, !_loading, () async {
                    ...
                    SizedBox(width: double.infinity, child: FilledButton.icon(
                      ...
                      icon: _loading ? const SizedBox(width: 16, height: 16, child: CircularProgressIndicator(strokeWidth: 2)) : const Icon(Icons.cloud_download),
    ...
    return AnimatedContainer(
      ...
      child: SingleChildScrollView(child: Column(children: [
        ...
        if (_toolbarExpanded) ...[
          ...
          _toolIcon('数据源/标的/周期/日期', Icons.search, _loading ? null : _openDataPanel),
          ...
          _toolIcon('严格逐K', Icons.play_circle_outline, _hasBars ? () => setState(() { _mode = 'step'; _cursor = math.min(120, _fullSnapshot.rawBars.length).toInt(); _snapshot = _sliceSnapshot(_fullSnapshot, _cursor); }) : null, selected: _mode == 'step'),
          ...
          const Divider(height: 18, color: Colors.white12),
          _toolIcon('左右放大', Icons.zoom_in, _hasBars ? () => _changeWindowSize(_windowSize - 15) : null),
          _toolIcon('左右缩小', Icons.zoom_out, _hasBars ? () => _changeWindowSize(_windowSize + 15) : null),
          _toolIcon('上下放大', Icons.keyboard_arrow_up, _hasBars ? () => _changePriceScale(_priceScale * 1.18) : null),
          _toolIcon('上下缩小', Icons.keyboard_arrow_down, _hasBars ? () => _changePriceScale(_priceScale / 1.18) : null),
          _toolIcon('重置缩放', Icons.center_focus_strong, _hasBars ? _resetChartZoom : null),
          _toolIcon('回到最新K线', Icons.my_location, _hasBars ? () => setState(() { _viewEndIndex = null; _crosshairIndex = null; }) : null),
        ],
      ])),
    );
  }

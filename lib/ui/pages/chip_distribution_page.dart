...
class _ChipDistributionPageState extends State<ChipDistributionPage> {
  ...
  List<ChipDistributionBar> get _onlineBars =>
      ChipOnlineReplayAdapter.fromSnapshot(_activeSnapshot);
  ...
  Widget _buildHeader(BuildContext context) {
    return Row(
      children: <Widget>[
        ...
        const Tooltip(
          message:
              '只使用在线 analyze_multi 返回的 K 线；不读取离线分笔文件。目标K优先级：step当前K > 十字线K > 视觉最右K。',
          ...
          children: <Widget>[
            ...
            if (_mode == 'step' && (_analysis?.frames.isNotEmpty == true))
              _LabeledSlider(
                ...
                display:
                    '${_safeFrameIndex + 1}/${_analysis?.frames.length ?? 0}',
            ...
            if (_mode != 'step' && bars.isNotEmpty) ...[
              _LabeledSlider(
                ...
                display:
                    _crosshairIndex == null ? '未激活' : '${_crosshairIndex! + 1}',
  ...
  Widget _buildMetricCard(
      ChipDistributionResult result, List<ChipDistributionBar> bars) {
    return DecoratedBox(
      decoration: BoxDecoration(
        color: const Color(0xFF131722),
        borderRadius: BorderRadius.circular(16),
          ...
          children: <Widget>[
            Wrap(
              ...
              children: <Widget>[
                ...
                _MetricChip(
                    label: '现价', value: result.currentPrice.toStringAsFixed(2)),
                _MetricChip(
                    label: '平均成本',
                    value: result.averageCost.toStringAsFixed(2)),
                _MetricChip(
                    label: '峰值价位', value: result.pocPrice.toStringAsFixed(2)),
                _MetricChip(
                    label: '获利筹码',
                    value: '${(result.profitRatio * 100).toStringAsFixed(1)}%'),
                _MetricChip(
                    label: '卖侧筹码', value: result.sellWeight.toStringAsFixed(0)),
                _MetricChip(
                    label: '买侧筹码', value: result.buyWeight.toStringAsFixed(0)),
              ],
            ),
            _LabeledSlider(
              label: '价格桶数',
  ...
  Future<void> _loadOnlineReplay() async {
    ...
    try {
      ...
      if (!mounted) return;
      final snap =
          _mode == 'step' && a.frames.isNotEmpty ? a.frames.first : a.snapshot;
      setState(() {
        _analysis = a;
        _frameIndex = 0;
        ...
        _viewEndIndex = null;
        _status =
            'loaded online bars:${_onlineBars.length} frames:${a.frames.length} active:$_activeLevel';
      });
    } catch (e) {
...
class _ChipDistributionPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final sellPaint = Paint()
      ..color = const Color(0xFF26A69A).withOpacity(0.62);
    ...
    for (var i = 0; i < result.bins.length; i++) {
      ...
      final totalWidth =
          maxRatio <= 0 ? 0.0 : chartWidth * (bin.ratio / maxRatio);
      final sellWidth =
          bin.weight <= 0 ? 0.0 : totalWidth * (bin.sellWeight / bin.weight);
      ...
      canvas.drawRRect(
        RRect.fromRectAndRadius(Rect.fromLTWH(chartLeft, top, sellWidth, h),
            const Radius.circular(2)),
        isPoc ? pocPaint : sellPaint,
      );
      canvas.drawRRect(
        RRect.fromRectAndRadius(
            Rect.fromLTWH(chartLeft + sellWidth, top, buyWidth, h),
            const Radius.circular(2)),
        isPoc ? pocPaint : buyPaint,
      );
    }

    final currentY = _priceToY(
        result.currentPrice, minPrice, maxPrice, chartTop, chartBottom);
    canvas.drawLine(Offset(chartLeft, currentY), Offset(chartRight, currentY),
        currentPaint);
    _drawText(canvas, '现价 ${result.currentPrice.toStringAsFixed(2)}',
        Offset(chartRight - 94, currentY - 18), const Color(0xFF66BB6A));
    _drawText(canvas, '价格', Offset(18, chartTop), Colors.white54);
    _drawText(canvas, maxPrice.toStringAsFixed(2), Offset(12, chartTop + 4),
        Colors.white70);
    _drawText(canvas, minPrice.toStringAsFixed(2), Offset(12, chartBottom - 18),
        Colors.white70);
    _drawText(canvas, '卖侧/买侧筹码权重', Offset(chartLeft, size.height - 18),
        Colors.white54);
  }
  ...
  @override
  bool shouldRepaint(covariant _ChipDistributionPainter oldDelegate) =>

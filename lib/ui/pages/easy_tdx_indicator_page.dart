...
class _EasyTdxIndicatorPageState extends State<EasyTdxIndicatorPage> {
  static const _indicatorOrder = [
    'KDJ', 'RSI', 'DMI', 'ATR', 'WR', 'CCI', 'BIAS', 'OBV',
    'PSY', 'TRIX', 'DPO', 'MTM', 'ROC', 'EXPMA', 'BBI', 'DFMA',
    'CR', 'KTN', 'XSII', 'VR', 'EMV', 'MASS', 'MFI', 'BRAR',
  ...
  int? _crossIndex;
  final Set<String> _shownIndicators = <String>{};
  ...
  List<String> get _availableIndicators {
    ...
    return [
      ...
      for (final item in keys)
        if (!_indicatorOrder.contains(item)) item,
    ];
  }

  Map<String, bool> get _showExtraMap => {for (final key in _availableIndicators) key: _shownIndicators.contains(key)};
  ...
  Widget _header() => Row(children: [
        ...
        const Text('动态渲染完整指标注册表', style: TextStyle(color: Colors.white54, fontSize: 12)),
        const Spacer(),
  ...
  Widget _switches() => _hScroll(
        ...
        Row(children: [
          ...
          for (final key in _availableIndicators)
            _chip(key, _shownIndicators.contains(key), (v) => setState(() => v ? _shownIndicators.add(key) : _shownIndicators.remove(key))),
  ...
  Widget _chip(String label, bool selected, ValueChanged<bool> onSelected) => Padding(
...
class _IndicatorPainter extends CustomPainter {
  ...
  void _cross(Canvas canvas, Size size, Rect main, double Function(int) x, double Function(double) py) {
    ...
    final lines = <String>[
      ...
      for (final e in showExtra.entries) if (e.value) '${e.key} ${_fmtMap(b.extra[e.key])}',
  ...
  void _text(Canvas canvas, String text, Offset offset, double size, Color color) {
    final tp = TextPainter(text: TextSpan(text: text, style: TextStyle(color: color, fontSize: size)), textDirection: TextDirection.ltr, maxLines: 1)..layout(maxWidth: 560);

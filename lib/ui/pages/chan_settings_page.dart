...
class _ChanSettingsPageState extends State<ChanSettingsPage> {
  ...
  Widget build(BuildContext context) {
    ...
    if (!widget.overlayMode) {
      return Scaffold(
        ...
        body: SafeArea(child: _settingsColumn(groups, invalid)),
      );
    }
          ...
          child: Stack(
            children: <Widget>[
              Positioned(
                ...
                bottom: 0,
                width: panelWidth,
                child: SafeArea(child: _settingsColumn(groups, invalid)),
              ),
              Positioned(
                left: panelWidth,
                top: 0,
              ...
              Positioned(
                ...
                top: 52,
                right: 16,
                child: IgnorePointer(child: _rightTooltipPanel()),
              ),
            ],
          ),
        );
  ...
  List<ChanSettingGroup> _visibleGroups() => ChanConfigStore.groups;
  ...
  Widget _settingTile(String key) {
    ...
    return MouseRegion(
      onEnter: (_) {
        if (_hoverKey != key) {
          setState(() => _hoverKey = key);
        }
      },
      child: Opacity(opacity: disabled ? 0.58 : 1.0, child: tile),
    );
  }
  ...
  String _hintFor(String key) {
    ...
    if (key.startsWith('macd') ||
        ...
        key == 'boll_n') {
      return '指标计算配置；详细逻辑见右侧说明。';
    }
    return 'chan.py 计算配置；详细逻辑见右侧说明。';
  }

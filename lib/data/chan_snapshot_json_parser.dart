...
class ChanSnapshotJsonParser {
  ...
  static ChanSnapshot parse(
    ...
    final rhythmLines =
        parseRhythmLines(data['rhythm_lines'] ?? data['rhythmLines']);
    final rhythmHits =
        parseRhythmHits(data['rhythm_hits'] ?? data['rhythmHits']);
    _addTiming(
        timing, '$timingPrefix.rhythm_1382', rhythmSw.elapsedMilliseconds);
  ...
  static RecursiveSEG? _parseRecursiveSeg(
      Map row, int index, int fallbackLayer) {
    final startRaw = _int(row['start_raw_index'] ?? row['startRawIndex']);
    final endRaw = _int(row['end_raw_index'] ?? row['endRawIndex']);
    final startPrice = _num(row['start_price'] ?? row['startPrice']);
    final endPrice = _num(row['end_price'] ?? row['endPrice']);
